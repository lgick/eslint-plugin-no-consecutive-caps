...
module.exports = {
  ...
  create(context) {
    ...
    const thisPropertiesToReport = new Map();
    ...
    return {
      Identifier(node) {
        ...
        if (
          ...
          if (suggestion) {
            if (!thisPropertiesToReport.has(name)) {
              thisPropertiesToReport.set(name, { suggestion, nodes: [] });
            }
            thisPropertiesToReport.get(name).nodes.push(node);
          }
          return;
        }
      ...
      'Program:exit'() {
        ...
        for (const [name, data] of thisPropertiesToReport.entries()) {
          const { suggestion, nodes } = data;
          const fix = fixer =>
            nodes.map(nodeToFix => fixer.replaceText(nodeToFix, suggestion));

          nodes.forEach(nodeToReport => {
            context.report({
              node: nodeToReport,
              messageId: 'consecutiveCaps',
              data: { name, suggestion },
